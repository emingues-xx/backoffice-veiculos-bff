# Teste de Vendas na Porta 3017
Write-Host "TESTE DE VENDAS - PORTA 3017" -ForegroundColor Green
Write-Host ""
...
try {
    ...
    Write-Host "   Servidor 3017 não está rodando: $($_.Exception.Message)" -ForegroundColor Red
    exit
}
...
$saleData = @{
    ...
    commission = 0
    paymentMethod = "cash"
    notes = "Teste de venda na porta 3017"
...
if ($saleId) {
    ...
    $updateData = @{
        status = "completed"
        notes = "Venda finalizada com sucesso na porta 3017"
    } | ConvertTo-Json
...
Write-Host ""
Write-Host "TESTE NA PORTA 3017 CONCLUIDO!" -ForegroundColor Green
