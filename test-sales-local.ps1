# Teste de Vendas Localmente
Write-Host "TESTE DE VENDAS LOCALMENTE - PORTA 3016" -ForegroundColor Green
Write-Host ""

# 1. Verificar se servidor local está rodando
Write-Host "1. Verificando servidor local..." -ForegroundColor Yellow
try {
    $health = Invoke-RestMethod -Uri "http://localhost:3016/health" -Method GET -TimeoutSec 5
    Write-Host "   Servidor local: $($health.message)" -ForegroundColor Green
} catch {
    Write-Host "   Servidor local não está rodando: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host "   Execute: npm run dev (na porta 3016)" -ForegroundColor Yellow
    exit
}
...
# 3. Listar veiculos para pegar um ID valido
Write-Host "3. Listando veiculos..." -ForegroundColor Yellow
try {
    $vehicles = Invoke-RestMethod -Uri "http://localhost:3016/api/vehicles" -Method GET -Headers $headers -TimeoutSec 10
...
try {
    $sales = Invoke-RestMethod -Uri "http://localhost:3016/api/sales" -Method GET -Headers $headers -TimeoutSec 10
...
$saleData = @{
    ...
    commission = 0
    paymentMethod = "cash"
    notes = "Teste de venda local"
} | ConvertTo-Json -Depth 3

try {
    $newSale = Invoke-RestMethod -Uri "http://localhost:3016/api/sales" -Method POST -Headers $headers -Body $saleData -TimeoutSec 10
...
if ($saleId) {
    ...
    try {
        $updatedSale = Invoke-RestMethod -Uri "http://localhost:3016/api/sales/$saleId" -Method PUT -Headers $headers -Body $updateData -TimeoutSec 10
    ...
    try {
        Invoke-RestMethod -Uri "http://localhost:3016/api/sales/$saleId" -Method DELETE -Headers $headers -TimeoutSec 10
...
try {
    $stats = Invoke-RestMethod -Uri "http://localhost:3016/api/sales/stats" -Method GET -Headers $headers -TimeoutSec 10
